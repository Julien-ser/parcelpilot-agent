...
## 0:35 to 1:25. Architecture (one slide or the README diagram)
...
3. **Policy engine and retrieval.** Three evaluators (cancellation, credit, SLA) return a
...
## 1:25 to 2:35. Demo 1: the example question, and why it is a trap

Signed in as **Ravi Menon** of Northstar. Ask:
...
Then, to show it isn't hardcoded, switch to **Sara Iyer** of LumenWorks and ask:
...
## 2:35 to 3:20. Demo 2: access control

Still as **Sara Iyer** of LumenWorks:
...
> that widens it. The tests call these accessors the way a jailbroken model
> would, asking directly by id, because that's the real threat model."

Switch to **Maya** on the ParcelPilot support team, ask the same question, show the full record
including the internal fields customers never see.

---

## 3:20 to 4:05. Demo 3: SLA, and the Sunday
...
## 4:05 to 4:40. Demo 4: proactive detection and the confirmation gate

Open **/ops**:

> "A reactive chatbot only helps once someone asks. This runs the same evaluators across
> every record. Two breached P1s, a credential exposure nobody has triaged for two
> hours. And my favourite: **ORD-2002 is owed INR 300 and there's no ticket.** The customer has
...
- [ ] Model provider has quota left. Groq caps tokens per day, Google caps requests per minute
